    ...
    <>
        ...
        <div className="mx-auto flex h-14 max-w-7xl items-center gap-3 px-4 md:px-6">
          ...
          {mock && (
            <span
              ...
              className="hidden rounded bg-accent-50 px-2.5 py-1 text-xs font-bold text-accent-700 sm:inline"
            >
      ...
      >
        ...
        <nav className="flex-1 space-y-4 px-3">
          {groups.map((group) => (
            <div key={group.title}>
              <p className="mx-3 mb-1.5 border-b border-slate-100 pb-1 text-xs font-bold tracking-wide text-slate-600">
                {group.title}
              </p>
              <div className="space-y-0.5">
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span className={active ? "text-brand-700" : "text-slate-600"}>
                        {item.icon}
                      </span>
                      <span className="min-w-0 flex-1">
                        ...
                        <span
                          className={`block truncate text-xs ${active ? "text-slate-500" : "text-slate-600"}`}
                        >
                          {item.desc}
                        </span>
                      </span>
        ...
        {mock && (
          <p className="mx-5 mb-3 rounded bg-accent-50 px-2.5 py-1 text-xs font-bold text-accent-700">
            Mock 모드 — Supabase 미연결
          </p>
        )}
      </aside>
...
function Brand() {
  return (
    <Link href="/" className="-mx-2 flex shrink-0 items-center gap-2.5 rounded-md px-2 py-2.5">
...
function Account({ email }: { email: string | null }) {
  if (!email) {
    return (
      <Link
        href="/login"
        className="shrink-0 rounded-md border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-500 hover:text-slate-900"
      >
        로그인
      </Link>
    );
  ...
  return (
    <div className="flex shrink-0 items-center gap-2.5">
      ...
      <form action={signOutAction}>
        <button
          type="submit"
          className="rounded-md border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-500 hover:text-slate-900"
        >
          로그아웃
        </button>
      </form>
