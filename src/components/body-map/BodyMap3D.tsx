  ...
  return (
    <Html position={position} center zIndexRange={[10, 0]}>
      <button
        ...
        /* 링 색은 brand-500을 rgba로 옮겨 적은 값이다. 임의 값 문법(shadow-[…])이라
           토큰이 안 닿으므로 **팔레트를 바꾸면 여기도 같이 고쳐야 한다** — 실제로
           에메랄드 시절 값이 남아 지도의 점만 초록으로 튄 적이 있다. */
        className={`group relative flex h-5 w-5 -translate-y-0 items-center justify-center rounded-full border-2 transition ${
          selected
            ? "scale-125 border-white bg-brand-500 shadow-[0_0_0_4px_rgba(58,96,137,0.35)]"
